...
<head>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { background: var(--bg-deep); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: var(--text-main); }
    ...
    .hdr-nav a:hover { border-color: var(--gold-dim); color: var(--gold-light); }
    ...
    /* Left panel */
    .left { width: 320px; border-right: 0.5px solid var(--border-subtle); overflow-y: auto; flex-shrink: 0; }
    .list-sec { padding: 10px 16px 6px; font-size: 10px; color: var(--gold-dim); letter-spacing: 0.08em; text-transform: uppercase; border-bottom: 0.5px solid var(--border-subtle); background: #181818; }
...
<div class="shell">
  ...
  <!-- Summary bar -->
  <div class="summary">
    <div class="stat s-total"><div class="stat-num">{{ stats.total }}</div><div class="stat-lbl">掃描封數</div></div>
    <div class="stat s-high"><div class="stat-num">{{ stats.high }}</div><div class="stat-lbl">高風險</div></div>
    <div class="stat s-med"><div class="stat-num">{{ stats.med }}</div><div class="stat-lbl">中風險</div></div>
    <div class="stat s-low"><div class="stat-num">{{ stats.low }}</div><div class="stat-lbl">安全</div></div>
    <div class="stat s-wl"><div class="stat-num">{{ stats.wl }}</div><div class="stat-lbl">白名單</div></div>
  </div>
  ...
  <div class="main">
    ...
    <div class="left">
      {% for group in email_groups %}
        <div class="list-sec">{{ group.category }}</div>
        {% for item in group.items %}
          <div class="email-item {% if item.global_idx == 0 %}active{% endif %}" onclick="showDetail({{ item.global_idx }})">
            <div class="risk-dot {{ item.dotClass }}"></div>
            <div class="item-content">
              <div class="item-subj">{{ item.list_subj }}</div>
              <div class="item-from">{{ item.list_from }}</div>
              <div class="item-score">{{ item.list_score }}</div>
            </div>
          </div>
        {% endfor %}
      {% endfor %}
...
<script>
// 後端傳入的資料集
const details = {{ raw_details | tojson }};
...
function showDetail(idx) {
  ...
  if (d.tags && d.tags.length > 0) {
    tagsHtml = `<div class="tag-row">${d.tags.map(t => `<span class="htag">${t}</span>`).join('')}</div>`;
  }

  let scoresHtml = '';
  if (d.scores && d.scores.length > 0) {
    scoresHtml = `<div class="score-row">${d.scores.map(s => `<div class="score-item">${s[0]} <span>${s[1]}</span></div>`).join('')}</div>`;
  }
  ...
  const activeItem = document.querySelectorAll('.email-item')[idx];
  if (activeItem) {
...
def index():
    ...
    raw_details = [
        ...
        {
            "badge": "⚠️ 中風險 · 60 / 100", "badgeClass": "badge-med",
            "subj": "Your password expires soon",
            "from": "來自：it-admin@company-internal.net",
            "explain": "此信件提醒密碼即將到期，雖使用公司常見內部範本，但域名非內部主要網域，需進一步確認。",
            "tags": ["⚠️ 密碼過期", "🔗 可疑連結"],
            "scores": [["規則引擎", "6分"], ["ML 機率", "52.0%"], ["HTML", "+2分"]],
            "action": "建議透過官方內部 Intranet 管道更改密碼，切勿點擊信件內的更新按鈕。",
            "ir": False, "irId": "", "sev": "", "impact": "", "actions": ""
        },
        {
        ...
        {
            "badge": "⚪ 白名單 · 略過分析", "badgeClass": "badge-low",
            "subj": "Your weekly recap is ready",
            "from": "來自：newsletter@lululemon.com",
            "explain": "此寄件者已加入系統信任白名單，自動跳過深度 AI 惡意特徵分析。",
            "tags": ["⚪ 白名單"],
            "scores": [["規則引擎", "0分"], ["ML 機率", "0.0%"], ["HTML", "0分"]],
            "action": "系統列為信任來源，可放心開啟。",
            "ir": False, "irId": "", "sev": "", "impact": "", "actions": ""
        }
    ]

    # 左側列表群組分類設定 (自動對應 raw_details 的索引)
    email_groups = [
        {
            "category": "高風險",
            "items": [
                {
                    "global_idx": 0,
                    "dotClass": "dot-high",
                    "list_subj": "URGENT: Bank account suspended",
                    "list_from": "security@bank-secure.xyz",
                    "list_score": "90/100 · 釣魚信件"
                },
                {
                    "global_idx": 1,
                    "dotClass": "dot-high",
                    "list_subj": "安全性快訊 — 立即驗證",
                    "list_from": "no-reply@accounts.google.xyz",
                    "list_score": "85/100 · 品牌偽裝"
                },
                {
                    "global_idx": 2,
                    "dotClass": "dot-high",
                    "list_subj": "退款通知 NT$3,200",
                    "list_from": "service@nhi-refund.biz",
                    "list_score": "88/100 · 詐騙簡訊"
                }
            ]
        },
        {
            ...
            "items": [
                {
                    "global_idx": 3,
                    "dotClass": "dot-med",
                    "list_subj": "You Left Something Behind",
                    "list_from": "no-reply@emails.skims.com",
                    "list_score": "55/100 · 商業行銷"
                },
                {
                    "global_idx": 4,
                    "dotClass": "dot-med",
                    "list_subj": "Your password expires soon",
                    "list_from": "it-admin@company-internal.net",
                    "list_score": "60/100 · 待確認"
                }
            ]
        },
        {
            ...
            "items": [
                {
                    ...
                    "dotClass": "dot-low",
                    "list_subj": "明天的組會時間確認",
                    "list_from": "prof.chen@ntu.edu.tw",
                    "list_score": "5/100 · 正常信件"
                },
                {
                    "global_idx": 6,
                    "dotClass": "dot-wl",
                    "list_subj": "Your weekly recap is ready",
                    "list_from": "newsletter@lululemon.com",
                    "list_score": "白名單 · 略過分析"
                }
            ]
        }
...
# 提供給 UptimeRobot 防止 Render 休眠的健康檢查點
@app.route('/healthz')
def health_check():
    return "OK", 200

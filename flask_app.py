# ============================================================
# AI 釣魚信件偵測系統 - Flask 網頁版 (雲端 Render & 本地雙用完整版)
# ============================================================
...
_ir_reports_store = {}  # 儲存資安事件 IR 報告供 JSON 匯出

# ── 2. 環境變數與設定 ────────────────────────────────────────
GROQ_API_KEY = os.getenv('GROQ_API_KEY', 'gsk_填入你的key')
NGROK_TOKEN  = os.getenv('NGROK_TOKEN', '填入你的ngrok_token')
...
print('⚡ 載入資料集並訓練支援中文分詞之 ML 模型...')
url = 'https://raw.githubusercontent.com/justmarkham/pycon-2016-tutorial/master/data/sms.tsv'
df = pd.read_csv(url, sep='\t', header=None, names=['label', 'text'])
X_train, X_test, y_train, y_test = train_test_split(
    df['text'], df['label'], test_size=0.2, random_state=42, stratify=df['label'])

vectorizer = TfidfVectorizer(tokenizer=chinese_tokenizer, max_features=3000, token_pattern=None)
...
def analyze_html(html_content):
    ...
    for link in links:
        ...
        if ('http://' in text or 'https://' in text) and text != href:
            score += 5
            findings.append(('偽裝連結', [f'文字:{text[:25]} -> 實際:{href[:25]}']))
        for d in ['xyz','biz','secure-','login-','verify']:
            if d in href.lower():
                score += 3; findings.append(('可疑網域', [href[:40]])); break

    imgs = soup.find_all('img')
    trackers = []
    for img in imgs:
        if str(img.get('width', '')) in ['1', '0'] or str(img.get('height', '')) in ['1', '0']:
            trackers.append(img.get('src', '')[:40]); score += 2
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header h1 { font-size: 18px; font-weight: 700; color: #ffffff; display: flex; align-items: center; gap: 10px; }
        ...
        h2 { font-size: 38px; font-weight: 700; color: #f9fafb; margin-bottom: 16px; letter-spacing: -0.02em; }
        .sub { font-size: 15px; color: #9ca3af; max-width: 500px; margin: 0 auto 36px; line-height: 1.6; }
        .btn { display: inline-flex; align-items: center; gap: 12px; background: #f9fafb; color: #030712; font-size: 14px; font-weight: 600; padding: 14px 28px; border-radius: 8px; text-decoration: none; transition: all 0.2s; box-shadow: 0 0 20px rgba(255, 255, 255, 0.15); }
        .btn:hover { background: #ffffff; transform: translateY(-2px); box-shadow: 0 0 30px rgba(255, 255, 255, 0.3); }
        ...
        .feat p { font-size: 12px; color: #9ca3af; line-height: 1.5; }
        .whitelist-note { background: rgba(16, 185, 129, 0.04); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: 10px; padding: 14px 20px; margin-top: 40px; font-size: 12px; color: #6ee7b7; text-align: left; line-height: 1.6; }
        .whitelist-note strong { color: #34d399; }
    </style>
</head>
<body>
    <div class="header">
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', 'Noto Sans TC', -apple-system, sans-serif; background-color: #07090e; color: #d1d5db; min-height: 100vh; line-height: 1.5; }
        ...
        .scan-time { font-size: 11px; color: #6b7280; font-family: 'JetBrains Mono', monospace; }
        .container { max-width: 880px; margin: 0 auto; padding: 32px 24px 60px; }
        .back { display: inline-flex; align-items: center; gap: 6px; background: rgba(255, 255, 255, 0.03); color: #38bdf8; font-size: 12px; padding: 8px 14px; border-radius: 6px; text-decoration: none; border: 1px solid rgba(56, 189, 248, 0.2); margin-bottom: 24px; font-family: 'JetBrains Mono', monospace; }
        
        .dashboard-grid { display: grid; grid-template-columns: 2fr 1fr; gap: 20px; margin-bottom: 32px; align-items: center; }
        .summary { background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; padding: 20px 24px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; text-align: center; }
        ...
        .card { background: rgba(15, 23, 42, 0.3); border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 10px; padding: 18px 20px; margin-bottom: 12px; position: relative; }
        ...
        .rb { font-size: 10px; font-weight: 600; padding: 3px 8px; border-radius: 4px; font-family: 'JetBrains Mono', monospace; }
        ...
        .tag { font-size: 11px; color: #fbbf24; background: rgba(251, 191, 36, 0.08); padding: 2px 7px; border-radius: 4px; border: 1px solid rgba(251, 191, 36, 0.2); font-family: 'JetBrains Mono', monospace; }
        .sc { font-size: 11px; color: #4b5563; margin-top: 10px; font-family: 'JetBrains Mono', monospace; }
        
        .ir { background: rgba(52, 211, 153, 0.03); border: 1px solid rgba(52, 211, 153, 0.2); border-radius: 6px; padding: 12px 14px; margin-top: 12px; }
        .ir-t { font-size: 12px; font-weight: 600; color: #34d399; display: flex; justify-content: space-between; }
        .ir-id { font-size: 11px; color: #6b7280; font-family: 'JetBrains Mono', monospace; }
        .ir-b { font-size: 12px; color: #a7f3d0; margin-top: 6px; line-height: 1.5; }
        .export-btn { background: rgba(52, 211, 153, 0.1); color: #34d399; border: 1px solid rgba(52, 211, 153, 0.3); padding: 2px 8px; border-radius: 4px; font-size: 11px; cursor: pointer; text-decoration: none; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="container">
        ...
        <div class="card h">
            ...
            {% if e.html_findings %}
            <div class="tags">{% for f in e.html_findings %}<span class="tag">{{f}}</span>{% endfor %}</div>
            {% endif %}
            <div class="sc">規則: {{e.rule_score}}分 | ML: {{e.ml_prob}} | HTML: +{{e.html_score}}分</div>
            {% if e.ir_id %}
            <div class="ir">
                <div class="ir-t">
                    <span>📄 IR 事件通報報告已自動產生</span>
                    <a href="/export_ir/{{e.ir_id}}" class="export-btn">匯出 JSON</a>
                </div>
    ...
    <script>
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [{{high}}, {{med}}, {{low}}, {{whitelist_count}}],
                    backgroundColor: ['#ff4d4d', '#fbbf24', '#34d399', '#4b5563'],
                    borderWidth: 0
                }]
            },
            options: {
...
if __name__ == '__main__':
    ...
    # 只有在非 Render 雲端環境（且有提供 NGROK_TOKEN）時才建立隧道
    if not os.getenv('RENDER') and NGROK_TOKEN and NGROK_TOKEN != '填入你的ngrok_token':
        ngrok.set_auth_token(NGROK_TOKEN)
        tunnel = ngrok.connect(5000)
        print(f'\n🌐 本地 Ngrok 網址：{tunnel.public_url}')
